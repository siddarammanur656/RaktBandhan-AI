...
def find_donors(payload: MatchRequest, current_user: dict = Depends(get_current_user)):
    ...
    # Secondary: distance_km (ascending)
    # Tertiary: reliability_score (descending)
    matched_donors.sort(key=lambda x: (not x["is_eligible"], x["distance_km"], -x["reliability_score"]))
